module.exports = {
  ...
  listMonitor: async (req, res) => {
    try {
      let students = await framework.services.students.basic.fetchWithLevel(req?.user);
      if (!students) {
        res.status(200).json({
          message: 'no records found!',
          error: false,
  ...
  deleteComment: async (req, res) => {
    try {
      let { id } = req.params;
      let comment = await framework.services.students.updateStudent.deleteComment(id);
      if (!comment) {
        res.status(400).json({
          message: 'invalid data',
          error: true,
