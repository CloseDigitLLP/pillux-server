      ...
      if (!debits.length) {
        ...
        let data = debits.map((student) => {
          ...
          return {
            ...
            email: student?.email,
            date_code: student?.date_code ? moment(student?.date_code).format('YYYY-DD-MM') : '',
  ...
  sendEmail: async (req, res) => {
    try {
      ...
      const msg = {
        ...
        text: `Hello, Dear Student - ${
          data.first + ' ' + data.lastname
        }, this is a remider for you need to pay your debited amount...`,
      ...
      sgMail
        ...
        .then(() => {
          ...
          res.status(200).json({
            ...
            error: false,
            data: `remider successfully send for student - ${data.firstname + ' ' + data.lastname} on email id was ${
              data.email
            }`,
          });
        })
